...
def main():
    ...
    global angle,circle_center,tracker
    ...
    pid_yaw = ctrl.PIDControler(width//2,1,0.1,0.0,40)
...
#            print( "Batt:",batt)
            ...
            if img is not None:
                ...
                do_movement(drone=drone,img=img,pid_yaw=pid_yaw)
        ...
        write(screen,f"output_pid_x:{pid_yaw.past_variables[-1]}",(width-200,40))
...
def do_movement(drone,img, pid_yaw):
   global angle,circle_center
   if not Track_Ball:
       ...
       output_pid_yaw = pid_yaw.compute(circle_center[0])
       print(output_pid_yaw)


       if not pid_yaw.reached_setpoint(circle_center[0]):
           angle += output_pid_yaw
       else:
           pid_yaw.reset()
       print(output_pid_yaw)

       if output_pid_yaw > 0:
           drone.rotate_clockwise(output_pid_yaw)
       else:
           drone.rotate_counter_clockwise(-output_pid_yaw)
